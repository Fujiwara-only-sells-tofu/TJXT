...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements ICourseService {
    ...
    public List<Integer> getCourseCount() {
        ...
        Integer count = this.lambdaQuery()
                .count();
        Integer shelfCount = this.lambdaQuery().eq(Course::getStatus, CourseStatus.SHELF.getStatus()).count();
        Integer downShelfCount = this.lambdaQuery().eq(Course::getStatus, CourseStatus.DOWN_SHELF.getStatus()).count();
        Integer noUpShelfCount = courseDraftMapper.selectCount(new LambdaQueryWrapper<CourseDraft>().eq(CourseDraft::getStatus, CourseStatus.NO_UP_SHELF.getStatus()));
        Integer finishedCount = this.lambdaQuery().eq(Course::getStatus, CourseStatus.FINISHED.getStatus()).count();
        countList.add(count);
        countList.add(shelfCount);
        countList.add(downShelfCount);
        countList.add(noUpShelfCount);
        countList.add(finishedCount);
        return countList;
    }
}
